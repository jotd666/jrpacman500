...
def process_mazes():
    ...
    with open("../src/maze_data.s","w") as fw:
        ...
        fw.write("\tdc.l\tmaze_{0}_dot_table_read_only,maze_{0}_wall_table,maze_6_misc,maze_{0}_bitmap\n".format(4))
...
def load_ripped_pics():
    big_pic = Image.open(os.path.join(this_dir,"Arcade - Jr Pac-Man - General Sprites.png"))
    maze_width = big_pic.size[0]//2 - 1
    maze_height = 240
    i = 1
    for x in [2,maze_width+2]:
        for y in range(2,maze_height*4,maze_height+2):
            if i < 8:
                maze = Image.new("RGB",(maze_width,maze_height))
                maze.paste(big_pic,(-x,-y))
                maze.save(os.path.join(dump_dir,f"maze_{i}.png"))
                i += 1

# load the big ripped pic from spriters resource

pic_dict = load_ripped_pics()
